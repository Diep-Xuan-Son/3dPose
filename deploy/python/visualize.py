...
def visualize_pose(imgfile,
    ...
    colors1 = [[255, 0, 0], [255, 0, 0], [255, 0, 0], [255, 0, 0], [255, 0, 0], [255, 0, 0], [255, 0, 0], \
    ...
    green_image = np.zeros((height_img,width_img,3), np.uint8)
    green_image[:,:] = (0,255,0)
    canvas = green_image.copy()
    # canvas = img.copy()
    for i in range(kpt_nums):
        for j in range(len(skeletons)):
            if skeletons[j][i, 2] < visual_thresh:
                continue
            if ids is None:
                color = colors1[i] if color_set is None else colors[color_set[j]
                                                                   %
                                                                   len(colors)]
            else:
